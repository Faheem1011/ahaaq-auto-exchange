...
export default function WindowTintingPage() {
  return (
    <main className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="space-y-4 max-w-3xl">
            ...
            <h1 className="text-4xl sm:text-6xl md:text-7xl font-black uppercase tracking-tight leading-[0.95]">
              ...
              <span className="text-zinc-400">Window Tinting</span>
            </h1>
      ...
      {/* Benefits Row */}
      <section className="py-16 bg-zinc-950 text-white px-6 md:px-8 border-t border-b border-zinc-800">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="space-y-2">
            <div className="w-10 h-10 rounded-xl bg-zinc-900 text-white flex items-center justify-center border border-zinc-800">
              <Sun size={20} />
            </div>
            <h3 className="font-bold text-sm uppercase">85% Solar Heat Block</h3>
            <p className="text-xs text-zinc-400 font-normal">Significantly reduces interior cabin temperature on hot summer days.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-400 font-normal">Protects passenger skin and prevents leather dashboard fading &amp; cracking.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-400 font-normal">Keeps valuables hidden from view and gives your vehicle a sleek profile.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-400 font-normal">Computer-cut patterns for exact glass fitment with zero hand-blade glass scratching.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-8 max-w-7xl mx-auto space-y-16">
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          <span className="text-xs font-black uppercase tracking-widest text-zinc-500">Automotive Tint Packages</span>
          ...
          {TINT_PACKAGES.map((pkg, i) => (
            <div
              ...
              className={`rounded-3xl p-8 flex flex-col justify-between space-y-6 transition-all ${
                pkg.popular
                  ? "bg-zinc-950 text-white border border-zinc-700 shadow-2xl scale-105"
                  : "bg-[#FAFAFA] text-zinc-950 border border-zinc-200"
              }`}
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${
                    pkg.popular ? "bg-white text-black" : "bg-zinc-200 text-zinc-800"
                  }`}>
                    {pkg.tag}
                  </span>
                </div>
                ...
                <ul className="space-y-2.5 pt-4 border-t border-zinc-800 text-xs font-normal">
                  {pkg.features.map((feat, fi) => (
                    <li key={fi} className="flex items-start gap-2">
                      <CheckCircle2 size={14} className={`shrink-0 mt-0.5 ${pkg.popular ? "text-white" : "text-zinc-900"}`} />
                      <span>{feat}</span>
                    </li>
                  ))}
                </ul>
