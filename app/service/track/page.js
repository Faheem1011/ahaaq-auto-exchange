...
function TrackRepairContent() {
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-8 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs uppercase tracking-widest rounded-2xl transition-all shadow-lg shrink-0 cursor-pointer"
          >
      ...
      {errorMsg && (
        <div className="max-w-2xl mx-auto p-5 bg-red-50 border border-red-200 rounded-2xl text-xs text-red-700 font-bold flex items-center gap-3">
      ...
      {approvalMessage && (
        <div className="max-w-2xl mx-auto p-5 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs text-emerald-800 font-bold flex items-center gap-3">
      ...
      {workOrders.length > 0 && (
        <div className="space-y-8 max-w-4xl mx-auto">
          {workOrders.map((wo) => {
            ...
            return (
              <div key={wo.id} className="bg-white border border-zinc-200 rounded-3xl p-6 sm:p-10 shadow-xl space-y-8 animate-in fade-in duration-300">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-zinc-200">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-zinc-500 font-medium">
                      Customer: <strong>{wo.customerName}</strong> • Department: <span className="uppercase">{wo.department}</span>
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-xs font-bold text-zinc-600">
                    ...
                    <span className="text-emerald-600 font-black">{wo.progressPercentage}% Complete</span>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-200 space-y-1">
                    ...
                    <p className="text-zinc-900 font-bold flex items-center gap-1.5">
                      <Wrench size={14} className="text-emerald-600" /> {wo.technicianName} ({wo.bayNumber})
                    </p>
                  </div>
                  <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-200 space-y-1">
                    <span className="text-zinc-400 font-bold uppercase tracking-wider block text-[10px]">Estimated Completion</span>
                    <p className="text-zinc-900 font-bold flex items-center gap-1.5">
                      <Clock size={14} className="text-emerald-600" /> {wo.estimatedCompletion ? new Date(wo.estimatedCompletion).toLocaleDateString() : "Updating shortly"}
                    </p>
                  </div>
                </div>
                ...
                {wo.publicStatusNotes && (
                  <div className="p-5 bg-emerald-50/60 rounded-2xl border border-emerald-200 text-xs text-emerald-950 space-y-1">
                    <strong className="font-bold uppercase tracking-wider text-[10px] text-emerald-800 block">Technician Update:</strong>
                    <p className="font-medium leading-relaxed">{wo.publicStatusNotes}</p>
                  </div>
                )}
                  ...
                  <div className="p-6 bg-zinc-950 text-white rounded-2xl space-y-6">
                    ...
                    {needsApproval ? (
                      <div className="p-5 bg-zinc-900 rounded-xl border border-zinc-800 space-y-4">
                        <div className="space-y-1">
                          <h4 className="text-xs font-black uppercase tracking-wider text-amber-400">Customer Authorization Required</h4>
                          <p className="text-xs text-zinc-300">Please review and digitally authorize our technicians to proceed with this repair.</p>
                        </div>
                        <div className="flex flex-wrap gap-3">
                          <button
                            ...
                            className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg flex items-center gap-1.5 cursor-pointer"
                          >
                      ...
                      <div className="flex items-center gap-2 text-xs text-emerald-400 font-bold">
                        <CheckCircle2 size={16} /> Work Authorized &amp; In Progress
                      </div>
                    )}
                  </div>
                )}
      ...
      <div className="max-w-2xl mx-auto p-6 bg-zinc-50 border border-zinc-200 rounded-3xl text-center space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 uppercase">Questions About Your Repair?</h3>
        <p className="text-xs text-zinc-500">Contact our service manager Bobby Ali directly for real-time workshop updates.</p>
        <div className="flex flex-wrap items-center justify-center gap-3 pt-1">
          <a href="tel:+19045029709" className="px-5 py-2.5 bg-zinc-900 text-white text-xs font-bold rounded-full hover:bg-zinc-800 transition-colors flex items-center gap-1.5">
            <Phone size={13} /> (904) 502-9709
          </a>
          <a href="https://wa.me/19045029709" target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-emerald-600 text-white text-xs font-bold rounded-full hover:bg-emerald-500 transition-colors flex items-center gap-1.5">
...
export default function TrackRepairPage() {
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* Hero */}
      <section className="pt-40 pb-16 px-6 md:px-8 bg-zinc-950 text-white text-center">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black uppercase tracking-tight text-white leading-tight">
            ...
            <span className="text-emerald-400">Repair Progress</span>
          </h1>
